...
- Current phase: Phase 6 navigation, metadata and automated QA implemented and live in production. Production route/content verification passed; wider browser/device, accessibility and consent QA remain open. All three sector pages accepted for progression. Phases 2, 3a-3d and the Phase 4 homepage are accepted for progression.
- Next action: Complete the remaining Phase 6 release QA: keyboard-only and reduced-motion review, consent rejection/persistence/withdrawal checks, narrow 320px coverage and physical-device/Safari checks where available. Production deployment no longer needs intervention.
...
| M04 | Cyber Essentials and ICO badges | Homepage trust strip | Retain existing badges; add supplied RISQS Verified badge as explicitly requested | Assurance strip | Existing label and badges retained; gold-outlined label above three 48px-high logos, including supplied RISQS Verified image; manual review pending |
...
| M13 | 242+ sites, 4+ sectors, 99.9% uptime, enterprise SLA | Homepage `.about__stats` | Retain values; animate numeric presentation on entry as requested | Evidence block; no invented or increased statistics | All final values retained; three numeric counters share a two-second clock and finish simultaneously; reduced-motion and static fallbacks preserved |
...
- Rechecked `https://www.apexiar.co.uk/` on 23 September 2026. The restructured homepage is live, including the new company positioning, four capabilities, pinned aerial-inspection sequence, protected certification/regulated-industries/testimonial sections and new sector routes.
- HTTPS HEAD checks returned 200 for the homepage, capabilities overview, all four capability pages, all three sector pages and the turbine inspection MP4. The host identifies as Amazon S3 behind CloudFront; no hosting setting or DNS change was made in this task.
- For all nine checked HTML routes, production content lengths and S3 ETags exactly matched MD5 hashes of the corresponding repository files after the host's CRLF-to-LF normalisation. This verifies the deployed HTML content against local `main`, not merely route availability. The multipart video ETag is not a simple MD5, but its production length exactly matches the local 13,805,629-byte asset and the route returns 200 with byte-range support advertised.
- Local `main`, `origin/main` and `origin/HEAD` are aligned at documentation commit `30ba645`; the approved site implementation remains commit `4c1543f`. Working tree was clean before this verification record. Deployment activation is resolved; wider Phase 6 browser/device, accessibility and consent follow-ups remain open. No application code, production configuration, dependency, commit or deployment changed in this task.

### Homepage capability heading and callout

- Changed only the homepage capability-section heading from "One connected technology ecosystem." to "Connected Technology Ecosystems" as requested. The four capability cards, their destinations, imagery and surrounding sections are unchanged.
- Added a dedicated non-interactive callout class to the existing supporting sentence. It reuses the Pause Video control's navy background, gold border/text and 6px radius, with the existing responsive heading layout retained.
- Files changed: `index.html`, `homepage.css`, this tracker and `WEBSITE-QA-REPORT.md`. No framework, dependency, asset, product route or protected content changed. Thirteen existing tests, JavaScript syntax, exact-text/static checks, Git whitespace and local Edge renders at 1440px, 768px and 390px passed. The callout stacks without overflow and all cards retain their existing responsive layouts. No commit or deployment.
- Follow-up copy refinement: changed only the Advisory & Deployment homepage-card description from "Practical expertise from engineering strategy to operations." to "Practical expertise from engineering strategy to assurance." The user will review the result manually; no rendered browser check requested. Exact-copy, preservation and Git whitespace checks passed. No commit or deployment.

### End-of-day release — 23 September 2026

- User explicitly requested save, commit and push to main for today's accumulated refinements. Scope: capability heading/callout and advisory copy; slowed gold timeline pulse; separate inspection introduction, skip removal, fitted video stage and compact closing caption; three-badge assurance layout with supplied RISQS image; synchronised numeric counters; tests and project records.
- Final 21 tests passed, both changed JavaScript files parsed, Git whitespace passed. Earlier task checks verified 101 homepage local references and four HTTP resources. No configured build/lint scripts. User opted for manual rendered review, so no new desktop takeover was performed. Fetched origin/main and confirmed zero divergence before commit. No force push intended. Next action after push: verify hosted deployment and resume any remaining Phase 6 QA in the next session.

### Assurance strip and synchronised counters (implementation)

- User requested a Pause Video-style gold container for Certified & Compliant, the badges beneath it and the supplied RISQS Verified logo at matching size. Homepage now has a centred stacked label/row; all three images use 48px height with preserved proportions. Source RISQS.png copied unchanged to assets/RISQS.png; SHA256 equality confirmed. M04/M30 updated.
- Added uptime to the existing counter group, with one decimal place. All three counters share a linear two-second animation and reach their exact final values on the same frame; floor-based updates prevent the small counter finishing early. Shared counter code also retains existing counters on other pages. Reduced motion and missing observer retain final values; enabling reduced motion during animation cancels it and restores final values. M13 updated; no claim values changed.
- Changed index.html, homepage.css, script.js, assets/RISQS.png, tests/about-counters.test.js and the two project records. All 21 tests, JavaScript syntax, Git whitespace, 101 local homepage references and homepage/CSS/JS/logo HTTP checks passed. No build/lint scripts configured. Manual desktop/tablet/mobile review left to user as requested; no desktop takeover, commit or deployment. Next action: user review of logo spacing and counter timing.

### Inspection introduction layout refinement (earlier work)

- Follow-up after user reported no visible improvement: the remaining empty band was the viewport-height stage around contained footage. Desktop stage height now uses the lesser of available viewport height or full-width footage height plus measured footer height. Media reserves the actual footer height; resize observation updates both when captions change. Stage and section share the same height variable, preserving scroll travel. Changed inspection.js/css, index.html version URLs, inspection tests and records. All 18 tests, syntax and whitespace passed, including a new tall-screen/short-screen/changed-caption geometry regression. Manual visual review remains with the user; no deployment.

- Closing-panel follow-up (M31): reduced interactive panel top padding from 48px to 16px and bottom padding to 16px; reduced workflow spacing to 10px. At the closing caption, the invisible approach text no longer reserves height and the closing text participates in normal layout. Reverse scrolling restores the approach layout. Dark backing maintains readability. Changed inspection.css, inspection.js, version links in index.html, existing inspection test assertions and project records. All 17 tests, syntax and whitespace checks passed. Manual visual review remains with the user; no build/lint scripts, commit or deployment.

- Width follow-up: removed the heading's 780px and supporting paragraph's 490px width caps so both use the full intro container and fit on individual lines at desktop widths. Smaller screens retain natural wrapping. CSS-only presentation change; user will visually review. Phase 6/M31 remain ready for manual review.

- User approved a compact navy introduction above the sticky footage, top-aligned video, and complete removal of the inspection skip option. Header copy is preserved and now scrolls away naturally; the protected horizon remains intact.
- M31 updated. Adjusted scroll start/travel for the header height and marker geometry for top-aligned contained video. Removed the skip handler and styles; maintained reduced-motion/loading/error fallbacks and dark-backed gold callouts. Versioned CSS/JS links for refresh.
- Changed index.html, inspection.css, inspection.js, tests/inspection.test.js, INSPECTION-NOTES.md and the two project records. All 17 tests passed, including eight inspection tests and desktop/tablet/mobile geometry simulations; syntax and whitespace checks passed. No configured build/lint scripts. Visual review left to the user as requested; next action is manual review of the introduction, pinning and final evidence. No commit or deployment.

### Operating-model sequential pulse (earlier work)

- Added a one-time, scroll-triggered purple pulse across the existing Sense / Connect / Understand / Act timeline. On first entry, the purple neon travels through each node and its line in order, then Act settles back to the original purple resting state.
- Progressive enhancement is preserved: without JavaScript or IntersectionObserver, the original static blue nodes and purple Act node remain. Reduced-motion visitors receive the same static state; changing the preference while the animation is prepared or running cancels it and restores that state.
